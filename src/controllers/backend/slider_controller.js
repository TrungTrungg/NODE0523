...
const { sliderService: service } = require('@services');
const { filterOptions, notify, sliderCollection: collection } = require('@utils');
...
const addOne = catchAsync(async (req, res) => {
    ...
    // } else {
    const { status, ordering, description } = req.body;
    let imageName = '';
    if (req.file) imageName = req.file.filename;

    await service.create(status.toLowerCase(), ordering, description, imageName);
...
const editOne = catchAsync(async (req, res) => {
    const { id, status, ordering, description } = req.body;
    ...
    // update
    await service.updateOneById(id, status.toLowerCase(), ordering, description, imageName);
